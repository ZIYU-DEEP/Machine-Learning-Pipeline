...
def _num_cat_split(features, num_list):
    """
    Split the features into two DataFrames: one is for the numeric, and the
    other is for the categorical.
    ...
    Returns:
        ...
        num (DataFrame): the numeric DataFrame
    """
    cat = features[[col for col in features.columns if col not in num_list]]
    num = features[[col for col in features.columns if col in num_list]]
    return cat, num
